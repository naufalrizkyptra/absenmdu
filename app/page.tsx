...
export default function Dashboard() {
  ...
  const checkLocation = () => {
    if (!navigator.geolocation) {
      toast.error('Browser anda tidak support GPS cuy!') 
      return
    }
    ...
    navigator.geolocation.getCurrentPosition(
      ...
      (error) => toast.error('Tolong izinin akses lokasi (GPS) buat absen ya!'), 
      { enableHighAccuracy: true }
    )
  }

  const JAM_MASUK_DEADLINE = "08:00"
  const JAM_PULANG_START = "14:00" // FIX: Pakai titik dua, bukan titik biasa
  ...
  const handleAbsen = async () => {
    ...
    if (!todayRecord) {
      ...
      if (!error) {
        ...
        toast.success(`Absen masuk sukses jam ${jamMenitSekarang}. Semangat kerjanya!`) 
      } else {
        console.error(error)
        toast.error("Gagal absen masuk euy!") 
      }
    } else {
      const { data, error } = await supabase
      .from('attendance')
      ...
      if (!error) {
        ...
        toast.success(`Berhasil absen pulang jam ${jamMenitSekarang}. Hati-hati di jalan!`) 
      } else {
                ...
                <div className="bg-white rounded-3xl shadow-sm border border-slate-100 p-5 md:p-6 relative overflow-hidden shrink-0">
                  ...
                  <div className="space-y-3">
                    ...
                    >
                      ...
                      todayRecord.check_out_time ? 'Tugas Selesai Hari Ini' : 
                      isBelumWaktunyaPulang ? `Belum Waktunya Pulang (${JAM_PULANG_START})` : 'Absen Pulang Sekarang'}
                    </button>
          ...
          {activeTab === 'tugas' && (
            <div className="bg-white rounded-3xl shadow-sm border border-slate-100 p-8 md:p-10 text-center h-[500px] flex flex-col items-center justify-center">
              <div className="w-20 h-20 bg-blue-50 rounded-full flex items-center justify-center mb-6">
                <svg className="w-10 h-10 text-blue-600" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 012-2h2a2 2 0 012 2m-6 9l2 2 4-4" /></svg>
              </div>
      ...
      <div className="md:hidden fixed bottom-0 left-0 right-0 bg-white border-t border-slate-100 pb-safe pt-2 px-6 z-30 shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.05)]">
        <div className="max-w-md mx-auto flex justify-between items-center pb-4">
          ...
          <button onClick={() => setActiveTab('tugas')} className={`flex flex-col items-center p-2 flex-1 transition-colors ${activeTab === 'tugas' ? 'text-blue-600' : 'text-slate-400'}`}>
            <svg className="w-6 h-6 mb-1" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 012-2h2a2 2 0 012 2m-6 9l2 2 4-4" /></svg>
